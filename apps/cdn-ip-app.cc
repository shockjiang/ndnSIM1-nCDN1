...
CDNIPApp::GetProducers() const
{
    ...
    NS_LOG_DEBUG("Producers are="<<str.str());
	return str.str();
}
...
CDNIPApp::AddProducers(NameComponents producer){
	//NameComponents source = NameComponents(prefix);
		...
		if (ix == m_Producers.size()) {
            ...
            NS_LOG_DEBUG("add Data provider "<<producer);
		}
...
CDNIPApp::OnContentObject (const Ptr<const ContentObjectHeader> &contentObject,
	...
	NS_LOG_DEBUG("ContentName="<<contentObject->GetName()<<". source="<<source);
	...
	vector<NameComponents>::size_type ix = 0;
		for (; ix !=m_Producers.size(); ++ix) {
			if (m_Producers[ix] == name) break;
				...
				NS_LOG_DEBUG("DataSource="<<m_Producers[ix]);
			}
		}
		...
		if (ix == m_Producers.size()) {
			...
			NS_LOG_DEBUG("index for producer"<<name<<" is "<<ix);
			return ix;
		}
...
CDNIPApp::SendPacket() {
	  ...
	  sent[t] += 1;
	  NS_LOG_DEBUG("additon after: sent["<<t<<"]="<<sent[t]);
